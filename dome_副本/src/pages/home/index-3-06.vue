		...
		<view class="page-view">
			...
			<view v-if="vehicleType != 1" class="card">
				<view class="card-icon-card-header">
					...
					<image class="card_icon_card_header_rt_map" :src="icoUrl + 'fw_card_icon.png'" mode="heightFix"
			...
			<!-- #endif -->
				...
				<!-- #endif -->
					...
					<view class="card-item">
						...
						<view v-else class="grid-box">
							<view class="grid-box-item" style="width: 140rpx" v-for="(item, index) in ywTabs"
								:key="index" @click="tabsClick(item)">
								<image class="grid-box-img" :src="item.iconUrl"></image>
				...
				<view class="card">
					...
					<view class="card-item">
						<view class="grid-box" style="justify-content: flex-start">
							<view class="grid-box-item" style="width: 140rpx" v-for="(item, index) in fwTabs"
								:key="index" @click="tabsClick(item)">
								<image class="grid-box-img" :src="item.iconUrl"></image>
								<text class="grid-box-text">{{ item.entryName }}</text>
							</view>
				...
				<view v-if="vehicleType == 1" class="card card-w">
					...
					<view class="hc-card" v-for="(item, index) in goodsorder" :key="index"
						...
						<view class="card_goods_list">
							<view class="card_goods_item" v-for="(item, index) in productList" :key="index"
								...
								<view class="card_goods_item_jf">
									...
									<image class="card_goods_item_jf_icon" src="/static/tabbar/tab4.png"></image>
								</view>
							</view>
						</view>
					</view>
			...
			<!-- <up-popup v-model:show="showExperiencePopup1" mode="center" :closeOnClickOverlay="true" :round="24"
				bgColor="transparent" z-index="9999999">
				<view class="exp-popup1" @tap.stop>
					<image class="exp-popup__card1" :src="imgUrl + 'home-zs-bj.png'" mode="widthFix" />
				</view>
			</up-popup> -->
			<!-- 订单二维码弹窗 -->
...
<script setup lang="ts">
	...
	import { getNewsList, recordShow, shouldShow, getRouteByLngLat, gridMenu, tiyanguan, gethomeconfig } from '@/api/home';
	...
	const showExperiencePopup1 = ref(true);
	...
	const jfTabs = ref([
		{
			name: '加入会员',
			icon: icoUrl.value + 'vip_icon.png',
			type: '',
		...
		{
			name: '绑定车辆',
			icon: icoUrl.value + 'bdcl_icon.png',
			type: '',
		...
		{
			name: '完善信息',
			icon: icoUrl.value + 'wsxx_icon.png',
			type: '',
	...
	const getyldj = async () => {
		try {
			const res = await gethomeconfig({ id: 20, enabled: 1 })
