<template>
	<view class='container-box'  :data-theme="pageTheme" >
		<up-sticky :bgColor="navBgColor">
			<!-- <up-navbar :title="detail.status == 2?'已评价':detail.status == 1?'已完成':'待服务'" bgColor='none' @leftClick="toBack" placeholder :fixed="true"
				:titleStyle="{fontWeight: 'bold'}"></up-navbar> -->
				<up-navbar title="预约详情" bgColor='none' @leftClick="toBack" placeholder :fixed="true"
					:titleStyle="{fontWeight: 'bold'}"></up-navbar>
		...
		<view class="container">
			...
			<view class="section">
				...
				<view class="info-item">
					...
					<text class="value">{{ detail.prCarBrandVo?.name }}</text>
				</view>
				<view class="info-item">
					<text class="label">车系</text>
					<text class="value">{{ detail.prCarSeriesVo?.name }}</text>
				</view>
				<view class="info-item">
			...
			<!-- 门店信息 -->
			<view class="section" 
			style="padding: 10rpx 37rpx 20rpx;"
			@click="navigateToApp(detail.prIndustry4sVo.name,detail.prIndustry4sVo.latitude,detail.prIndustry4sVo.longitude)">
				<view class="section-title" style="width: 100%;padding-bottom: 20rpx;border-bottom: 1px solid #ebebeb;">门店信息</view>
				<view class="store-info">
					...
					<view class="store-details">
						...
						<view class="bdphone" v-if="detail.prIndustry4sVo.contactNumber"  @click.stop="makePhoneCall(detail.prIndustry4sVo?.contactNumber)">
							拨打商家电话
						</view>
					</view>
				</view>
...
<script setup lang="ts">
	...
	import { getPrAppointmentOldCar,updatePrAppointmentOldCar } from '@/api/oldCar';
	const detail = ref({});
	...
	const updateData = async ()=>{
		try{
			let params = {...detail.value,status: 1}
			let res = await updatePrAppointmentOldCar(params);
			if(res.code == 200){
				toast('修改成功')
	...
	const getDetail = async (id) => {
		let res = await getPrAppointmentOldCar(id); 
		if(res.code == 200){
