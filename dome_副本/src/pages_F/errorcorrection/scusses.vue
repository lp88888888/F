		...
		<view class="container">
			<up-sticky>
				<view class="searchView">
					<up-navbar title="信息纠错提交" leftIconColor="#000000" bgColor="transparent" @leftClick="toPath"
			...
			<view class="success-top">
				...
				<text class="main-title">纠错信息已提交</text>
				<text class="sub-desc">感谢您的反馈，我们将尽快核实信息并更新收费站状态</text>
			</view>

			<!-- 提交信息卡片 -->
			<view class="info-card">
				<view class="card-header">
					<text class="card-title">提交信息</text>
					<text class="status-tag">{{formatCardStatus(tollStatiobj.auditStatus)}}</text>
				</view>
				<view class="info-item">
					<text class="item-label">收费站：</text>
					<text class="item-value">{{ tollStatiobj.tollStationName }}</text>
				</view>
				<view class="info-item">
					<text class="item-label">错误类型：</text>
					<text class="item-value">{{formatCardStatusStr(tollStatiobj.submitOperationalStatus) }}</text>
				</view>
				<view class="info-item">
					<text class="item-label">提交时间：</text>
					<text class="item-value">{{tollStatiobj.createTime }}</text>
				</view>
			</view>

			<!-- 奖励提示卡片 -->
			<view class="reward-card">
				<!-- 礼物图标：可替换为自己的图片/uni-icons图标 -->
				<view class="gift-icon">
					<image :src="imgUrl + 'scusses-lw.png'" mode="aspectFit" class="gift-img"></image>
				</view>
				<view class="reward-content">
					<text class="reward-title">参与反馈得奖励</text>
					<text class="reward-desc">您的反馈经核实有效后，将获得{{point}}积分奖励，可在积分商城兑换精美礼品~</text>
				</view>
			</view>

			<!-- 底部返回按钮 -->
			<view class="btn-bar" @click="toPath()">
				返回
			</view>
		</view>
	</view>
