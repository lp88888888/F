<template>
    ...
    backgroundImage:  `url(${imgUrl}vipcard/qiyeminxi-bj-icon.png)`,
    backgroundSize: 'cover',
    backgroundPosition: 'center',
    backgroundRepeat: 'no-repeat'
  }">
			<view class="points-opk">
				<view style="font-weight: 600;font-size: 36rpx;color: #FFFFFF;">{{userinfo.nickName}} <text
						style="font-weight: 400;font-size: 24rpx;color: #FFFFFF;">{{enterlist[0]?.enterpriseName}}
						等<text style="color: #C5C188;"> {{enterlist.length}}</text>家公司</text></view>
				<view style="font-weight: 400;font-size: 24rpx;color: #FFFFFF;">企业积分:<text
						style="font-weight: 600;font-size: 36rpx;color: #FFFFFF;">{{userinfo.enterprisePointsAccount.commonPoint}}</text>
				</view>
			</view>
		</view>
		...
		<view class="filter-tabs">
			<view style="display: flex;align-items: center;">
				<view class="date-filter" @click="show=true">
					...
					<up-icon name="arrow-down-fill" color="#000" size="14"></up-icon>
				</view>
			</view>
			<view>
			...
			<view class="integral-list">
				<view v-for="(item, itemIndex) in pointslist" :key="itemIndex" @click="todal(item)">
					<view class="integral-item">
						<view class="date">{{formatTime(item.validStartTime)}}</view>
						<view>
							<view class="detail-item">
								<view class="">
									<text class="name">{{item.pointsDetail }}</text><br />
									<text class="time">{{ item.validStartTime }}来源:{{item.pointsSourceName}}</text>
								</view>
								<view class="">
...
<script setup lang="ts">
	...
	const formatTime = (dateInput : string | number | Date) => {
		...
		if (inputDate.getTime() === today.getTime()) {
			...
			return '昨天'
		} else if (input.getTime() === beforeYesterday.getTime()) {
			return '前天'
		} else {
