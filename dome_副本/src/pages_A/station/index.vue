	...
	<view class="container" :data-theme="pageTheme">
		...
		<view class="search">
			...
			<!-- <view class="search-box">
				<up-input placeholder="请输入关键字搜索" v-model="searchTex" shape="circle" height="35" @change="inputConfirm">
					<template #prefix>
			...
			<view class="searchBg-s">
				...
				<input v-model="searchTex" class="texSearchTip-s" placeholder="请输入服务区关键字" type="text"
		...
		<view class="listView" :style="{ height: boxHeight + 'rpx' }">
			...
			<scroll-view :scroll-y="true" class="listView-scroll" @scrolltolower="scrolltolower">
				<view class="service-card" v-for="(item, index) in dataList" :key="index" @click="goStation(item.ids)">
				...
				<!-- <view class="s-view" v-for="(item, index) in dataList" :key="index" @click="goStation(item.ids)">
					<view class="cont">
						<view class="cont-img">
							<image class="image" :src="item.serviceAreaPicture || `${imgUrl}service-list-default.png`" lazy-load />
						</view>
						<view>
							<view class="texNoWrap" :style="{ marginBottom: !item.detailList || item.detailList.length === 0 ? '36rpx' : '' }">
								{{ item.serviceAreaName }} {{ handleEstimate(item) }}
							</view>
							<view class="texNoWrap" :style="{ margin: !item.detailList ? '0' : '' }">
								{{ item.address }}
							</view>
							<view v-if="item.detailList && item.detailList.length > 0">
								<image v-for="(subItem, index2) in item.detailList" :key="index2" :src="subItem.serviceInfoPicUrl" />
							</view>
						</view>
						<view class="map-icon">
		...
		<PageDurationTracker pageName="服务区" type='count'/>
	</view>
</template>
...
const getList = async (lat, lon) => {
	...
	var params = {
		...
		serviceAreaName: searchTex.value ? searchTex.value : '',
		operationalStatus: merTypes.value[merIndex.value].key,
	};
	showLoading();

	try {
		let res: any = await getgetServiceAreaPageList(params);
...
const handleMarkerTap = (event) => {
	...
	// return
	uni.navigateTo({
		url: '/pages_A/stationDetails/index?id=' + filterMaker.ids,
	});
};
