		...
		<view class="pageView" style="padding: 32rpx;">
			<view class="carPad cards">
				<view class="kdView">
					<view class="kdView-text">
						<view class="kdView-text-left">
							...
							<text style="margin-right: 10rpx;margin-left: 20rpx;">{{detail.logisticsName}}</text>
							<text>{{detail.logisticsNo}}</text>
						</view>
						<view class="kdView-text-cop" @click="copyText(detail.logisticsNo)">复制</view>
					</view>
				</view>
				...
				<view class="kdView-text-address">
					...
					<text
						class="kdView-text-address-t">送至:{{detail.receiverAreaName}}{{detail.receiverDetailAddress}}</text>
				</view>
				<view class="kdView-text-name">
					<text>{{detail.receiverName}}:</text><text>{{detail.receiverMobile}}</text>
				</view>
			</view>
...
<script setup lang="ts">
	...
	import { getpayOrderInfo, getmapTrack, getOrderInfo } from '@/api/points';
	import {
		useTheme
	...
	const gettrack = async () => {
		const res = await getmapTrack(detail.value.logisticsNo);
	...
	const getData = async (orderId) => {
		...
		try {
			...
			if (res.code == 200) {
				...
				gettrack()
				if (res.data.logisticsTracking) {
					let arr = JSON.parse(res.data.logisticsTracking.status).reverse()
					for (let i in arr) {
						locList.value.push({ ...res.data.logisticsTracking, statusCeshi: arr[i] })
					}
