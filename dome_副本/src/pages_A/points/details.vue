<template>
	<view class="container" :data-theme="pageTheme">
		<view class="swiperBox">
			<up-swiper :list="swiperList" height="350" @change="(e) => (currentNum = e.current)" :autoplay="false"
		...
		<view class="content">
			<view class="con-header flex just-sb ali-cen colf">
				<view class="flex ali-cen" style="flex-direction: column;align-items: self-start;">
					<view class="flex ali-cen">
						<image class="goods_item_img-1" v-if="pointsDetail.returnActivityType==4"
							:src="imgUrl + 'zhekou-4.png'" mode="">
						</image>
						<text class="fs-50 m-r-15">{{ detailsPoint }}</text>
						<text class="fs-28">积分</text>
						<text class="fs-28" style="text-decoration: line-through;color:#f0f0f0;"
							v-if="pointsDetail.returnActivityType==4||pointsDetail.returnActivityType==3">{{pointsDetail.point}}</text>
					</view>

					<view class="del-box" v-if="pointsDetail.returnActivityType==3">
						<image class="goods_item_img-2" :src="imgUrl + 'miaosha-2.png'" mode="">
						</image>
						<text class="del-box-text">限时秒杀</text>
					</view>
					<view class="del-box" v-if="pointsDetail.returnActivityType==1">
						<image class="goods_item_img-2" :src="imgUrl + 'fanquan-2.png'" mode="">
						</image>
						<text class="del-box-text">消费返优惠券</text>
					</view>
				</view>
				<view class="miansha-box"
					v-if="pointsDetail.returnActivityType==3||pointsDetail.returnActivityType==1||pointsDetail.returnActivityType==4">
					<image class="goods_item_img-3" :src="imgUrl + 'miaosha-4.png'" mode="">
					</image>
					<text>距结束</text>
				...
				<text class="fs-24"
					v-if="pointsDetail.returnActivityType!=3&&pointsDetail.returnActivityType!=1&&pointsDetail.returnActivityType!=4"
					style="color: #fff">已上架{{ totalStock }}</text>
				<text class="fs-24 upnum-text"
					v-if="pointsDetail.returnActivityType==3||pointsDetail.returnActivityType==1||pointsDetail.returnActivityType==4"
					style="color: #fff">已上架{{ totalStock }}</text>
			</view>
			<view class="con-info">
				...
				<view class="m-t-25 fs-26 col9 fw-400 lh40" style="display:flex;justify-content: space-between;">
					...
					<!-- <view class="tab-fenxiang" @click="shearOpen()">
						<image class="goods_item_img-4" :src="imgUrl + 'fenxiang-1.png'" mode="">
						</image>
						<text>分享</text>
					</view> -->
				</view>
			...
			<!-- 资讯模块 -->
			<view class="news_box" v-if="newsinfo.title">
				<view class="news_title" @click="toNav(`/pages_A/points/newsdetail?id=${newsinfo.id}`)">
					<uni-icons type="sound-filled" color="#3D3D3D" size="24"></uni-icons>
					<text>{{newsinfo.title}}</text>
				</view>
			</view>
			<view class="zslw-box" v-if="pointsDetail.allowGive==1">
				<!-- <view class="zslw-box"> -->
				<view class="zslw-box-1">
					<image class="goods_item_img-7" :src="imgUrl + 'liwuhezi-icon.png'" mode="">
					</image>
					<text class="zslw-box-text1">支持赠送礼物</text>
				</view>
				<view class="zslw-box-2" @click="toNav('/pages_A/points/givegift?id='+detailId)">
			...
			<view class="tab-box">
				<view class="tab-container">
					<view v-for="(item, index) in tabList" :key="index" class="tab-item"
						:class="{ active: currentTab == item.id }" @click="currentTabFun(item.id)">
						{{ item.name }}
					</view>
				</view>
				...
				<view class="eva-item flex flex-col m-t-30" v-for="(item, index) in sysReviewVoList" :key="index">
					...
					<view class="posView">
						...
						<view class="eva-img-box flex" v-if="item.imageUrls.length">
							<image class="eva-img" v-for="(url, inx) in item.imageUrls" :key="inx" :src="url"
								@click="$previewListImage(inx, item.imageUrls)" mode="">
							</image>
						</view>
					</view>
				</view>
		...
		<view class="bottom-bar flex ali-cen just-sb">
			<view class="foot_cz">
				...
				<view class="pr flex flex-col ali-cen" @click="toJump('/pages_A/shoppingCart/index')">
					...
					<view v-if="cartCount > 0" class="dot"
						:class="['dot', cartCount < 10 ? 'dot--1' : cartCount < 100 ? 'dot--2' : 'dot--3']">
						{{ cartCount > 99 ? '99+' : cartCount }}
					</view>
				</view>
				<view class="btn-box">
					<up-icon name="star" size="24" v-if="collect" @click="addcreateFun()"></up-icon>
					<up-icon name="star-fill" :color="themeStore.themeColors.$primaryColor" size="24" v-else
						@click="deletecreateFun()"></up-icon>
					<view class="fs-22 col3">收藏</view>
				</view>
				<view class="btn-box" @click="shearOpen()">
					<up-icon name="share-square" size="24"></up-icon>
					<view class="fs-22 col3">分享</view>
				</view>
			</view>
			<view class="btn-group">
				<view class="btn-cart" v-if="campaignId==''" @click="openSheet(1)">加入购物车</view>
		...
		<ExchangeSheet ref="sheetRef" :totalStock="totalStock" @confirm="onConfirm" @onSku="changeSku"
			:couponFlag="couponFlag" />
		<!-- <Share v-if="detail.id && pointsDetail.returnActivityType==3" ref="shearRef" pageType="mx" :itemObj="detail" :invitationId="detailId"></Share> -->
		<Share2 v-if="detail.id" ref="shearRef" pageType="mx" :itemObj="detail" :invitationId="detailId"></Share2>
	</view>
</template>
<script setup lang="ts">
	...
	import { onLoad, onReachBottom, onShow, onShareAppMessage, onShareTimeline } from '@dcloudio/uni-app';
	import { toNav, toBack } from '@/utils/route';
	import Share from '@/components/Share/indexzs';
	import Share2 from '@/components/Share/index';
	...
	const swiperList = computed(() => {
		if (list6.value.length) return list6.value
		return list7.value
	});
	// 详情ID
	const detailId = ref('');
	...
	const openSheet = async (type : string | number) => {
		...
		if (type == 1 && pointsDetail.value.returnActivityType == 3) {
			...
			if (pointsDetail.value.returnActivityType == 3 && detail.value.skus[0].canUseActivity == 0) {
				...
				console.log(campaignId.value,'campaignId.value')
				sheetRef.value?.open(detail.value, type, detailId.value, pointsDetail.value.returnActivityType,campaignId.value);
			}
		}
	};
	...
	const getDetail = async (id : string) => {
		...
		if (res.code == 200) {
			...
			if (res.data.returnActivityType == 3 || res.data.returnActivityType == 4) {
				...
				totalStock.value = res.data.activityStock;
			} else {
			...
			if (res.data.productSpuDetailRespVO) {
				...
				detail.value = res1;
				if (res.data.returnActivityType == 3) {
					// activityEndTime=2026-03-21 20:22:23
				...
				if (res.data.returnActivityType == 1) {
					const endTimeStr = res.data.activityEndTime; // 从接口 res.data.endTime 获取
				...
				if (res.data.returnActivityType == 4) {
					const endTimeStr = res.data.activityEndTime; // 从接口 res.data.endTime 获取
	...
	onLoad((opticon) => {
		...
		if(opticon.campaignId){
			campaignId.value = opticon.campaignId;
		}
		if (opticon.scene) {
