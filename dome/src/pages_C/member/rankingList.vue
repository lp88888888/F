<template>
	...
	<view class="page" :data-theme="pageTheme">
		...
		<view class='pags-box'>
			<view class="date-sai">
				<view class="date-filter" @click="monthshow=true">
					<text>{{monthsai}}月</text>
					<up-icon name="arrow-down-fill" color="#000" size="9"></up-icon>
				</view>
				<view class="date-filter" v-if="userinfo.memberType!=1" @click="pickshow=true">
			...
			<view class='rank-box'>
				<view class="top-rank-container" v-if="top3Items.length>0">
					...
					<view class="rank-item rank1 " v-if="top3Items.length>=2">
						...
						<view class="username">{{ top3Items[1]?.nickname}}</view>
					...
					<view class="rank-item rank2 " v-if="top3Items.length>=1">
						...
						<view class="username">{{ top3Items[0]?.nickname}}</view>
					...
					<view class="rank-item rank3 " v-if="top3Items.length>=3">
						...
						<view class="username">{{ top3Items[2]?.nickname}}</view>
			...
			<view class="rank-list-container">
				...
				<view class="header" v-if="otherItems.length>0">
					...
					<view v-if="currentIndex==0" class="col3">获得积分</view>
					<view v-if="currentIndex==1" class="col3">消耗积分</view>
					<view v-if="currentIndex==2" class="col3">行驶里程</view>
					<view v-if="currentIndex==1" class="col4">即将奖励</view>
				</view>
				...
				<view class="list" v-if="otherItems.length>0">
					<view v-for="(item, index) in otherItems" :key="index" class="list-item"
						...
						<view class="user">
							...
							<text v-if="currentIndex==2" class="name">{{ item.licensePlate }}</text>
						</view>
						<view v-if="currentIndex==0" class="score">{{ item.totalPoints }}</view>
						<view v-if="currentIndex==2" class="score">{{ item.totalMileage }}</view>
						<view v-if="currentIndex==1" class="score">{{ item.totalPoints }}</view>
						<view v-if="currentIndex==1" class="score">{{ item.rewardPoints }}</view>
					</view>
			...
			<view v-if="currentIndex==2 && myponklist.length>0">
				<view class="my-rank-tip" v-for="(item, index) in myponklist" :key="index">
					<view class="label">我的名次</view>
					<view class="content">
						<image :src="item.avatar" mode="aspectFill" class="avatar-small"></image>
						<text class="username">{{ item.nickname }}</text>
						<text class="score">行驶{{ item.totalMileage }}KM</text>
						<text class="status">{{ item.ranking||'未进TOP100' }}</text>
					</view>
				</view>
			</view>
		...
		<up-datetime-picker :maxDate="maxtime" :show="monthshow" format='YYYY-MM' v-model="monthsai"
			@confirm="onConfirm" @cancel="monthshow = false" mode="year-month"></up-datetime-picker>
		<up-picker :show="pickshow" :columns="columns" @confirm="confirmpick" @cancel="pickshow=false"></up-picker>
	</view>
</template>
...
<script setup lang="ts">
	...
	const tabs = ref(['积分获得榜', '积分消耗榜', '里程榜单']);
