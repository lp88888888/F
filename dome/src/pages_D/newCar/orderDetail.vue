<template>
	<view class='container-box' :data-theme="pageTheme">
		...
		<!-- 	<up-navbar :title="detail.status == 2?'已评价':detail.status == 1?'已服务':'待服务'" bgColor='none'
			...
			<view class="container">
				...
				<view class="section">
					...
					<view class="info-item">
						...
						<text class="value">{{ detail?.carBrandName }}</text>
					</view>
					<view class="info-item">
						<text class="label">车系</text>
						<text class="value">{{ detail?.carSeriesName }}</text>
					</view>
					<view class="info-item">
...
<script setup lang="ts">
	...
	import { selectDetailById } from '@/api/car';
	const detail = ref({});
	...
	const toEvaluate = () => {
		toNav(`/pages_D/usedCar/orderEvaluate?id=${detail.value.id}&merchantId=${detail.value.prIndustry4sVo?.id}&type=1`)
	}

	const getDetail = async (id) => {
		let res = await selectDetailById(id);
		if (res.code == 200) {
