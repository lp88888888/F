			...
			<view class="card-header">
				<view class="user-info">
					...
					<view class="user-base">
						<text class="user-name">{{objData.name}}</text>
						<text class="user-id" v-if="objData.auditStatus==1">编号{{objData.id}}</text>
					</view>
				</view>
				<image :src="imgUrl+'static/fb-fx.png'" class="store-image" v-if="objData.auditStatus==1"></image>
			</view>

			<!-- 票数和排名区域 -->
			<view class="vote-info" v-if="objData.auditStatus==1">
				<text class="vote-count">已获得：{{objData.voteCount}}票</text>
				<view class="rank-wrap">
					<text class="rank-text">当前排名：第{{objData.ranking?objData.ranking:'/'}}名</text>
					<text class="rank-gap">距上一名还差{{objData.gapFromPrevious?objData.gapFromPrevious:'0'}}票</text>
				</view>
			</view>

			<!-- 底部：发布时间、审核状态 -->
			<view class="card-footer">
				<text class="publish-time">发布时间：{{objData.approvalTime}}</text>
				<text class="audit-status" v-if="objData.auditStatus==1">审核通过</text>
				<text class="audit-status2" v-if="objData.auditStatus==2">已驳回</text>
				<text class="audit-status1" v-if="objData.auditStatus==0">待审核</text>
			</view>
		</view>
		<view style="width:90%;margin: 0 auto 10rpx; font-size: 24rpx;" v-if="objData.auditStatus==2">
			<view style="display: flex;justify-content: space-between;align-items: center;margin: 20rpx 0;">
				<view>审核时间:</view>
				<view style="color: #6B7073;">{{objData.approvalTime}}</view>
			</view>
			<view style="display: flex;justify-content: space-between;align-items: center;margin: 20rpx 0;">
				<view style="width: 380rpx;">驳回原因:</view>
				<view style="color: #FF0000;">{{objData.rejection}}</view>
			</view>
		</view>
		<view class="box">
			<!-- 表单区域 - 调整外层间距 -->
			<view class="form-wrap">
				<u-form ref="formRef" :model="formData" class="form-container">
					...
					<view style="width: 100%; background: #fff;padding:30rpx 30rpx; margin-bottom: 30rpx; box-sizing: border-box;box-shadow: 0rpx -6rpx 16rpx 0rpx rgba(0,0,0,0.04);border-radius: 20rpx;">
						<!-- 7. 姓名 -->
						<u-form-item label="姓名" required required-color="#f00" label-position="left" label-width="auto"
							class="form-item">
							<u-input v-model="formData.name" placeholder="请输入" class="input-style" maxlength="15"
								disabled></u-input>
						</u-form-item>
						...
						<u-form-item label="联系方式" required required-color="#f00" label-position="left"
							label-width="auto" class="form-item">
							<u-input v-model="formData.phone" placeholder="请输入" class="input-style" maxlength="11"
								disabled></u-input>
						</u-form-item>
					</view>
				</u-form>
