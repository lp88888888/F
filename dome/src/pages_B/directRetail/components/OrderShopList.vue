<template>
	<view class="shopCard">
		<view class="shopCard-title">商品信息</view>
		<view>
			<view v-for="(item,index) in dataList" :key="index">
				<view class="shopItem">
					<image class="shopItem-image" :src="item.pic" mode=""></image>
					<view class="shopItem-center">
						<view class="shopItem-center-name">{{item.spuName}}</view>
						<view class="shopItem-center-nums">数量:{{item.count}}</view>
					</view>
					<view class="shopItem-price">
						<view class="newPrice"><text
								class="fs-24">￥</text>{{decimalPointDivision(item.spuTotalAmount,'left')}}<text
								class="fs-28">{{decimalPointDivision(item.price,'right')}}</text></view>
					</view>
				</view>
			</view>
