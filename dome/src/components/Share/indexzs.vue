<template>
	<view>
		...
		<u-popup mode="bottom" round="10" bgColor="transparent" :safeAreaInsetBottom="false" :show="show" @close="close"
			...
			<view>
				<view class="fl-ce-ce"
					style="flex: 1;height: 588px;flex-direction: column;justify-content: flex-start;">
...
<script setup>
	...
	const posterData = computed(() => {
		...
		if (props.pageType == 'scenicSpotTicket') {
			obj = {
				...
				price: props.itemObj.productGroupVoList[0].odsJjbProductVoList[0].odsJjbStockVo.settlePrice ||
					'',
				QrCodeObj: {
			...
			obj = {
				...
				goodsImg: props.itemObj.productSpuDetailRespVO.picUrl, // 商品图
				mobile: props.itemObj.phone,
				goodsName: props.itemObj.productSpuDetailRespVO.name,
				QrCodeObj: {
					scene: `id=${1}`,
					// page:'/pages_A/login/login',
					check_path: false,
	...
	onShareAppMessage((res) => {
		...
		if (props.pageType == 'zs') {
			path = 'pages/home/home'
			imageUrl = props.itemObj.productSpuDetailRespVO.picUrl || ''
		}
		
		console.log('-0-0-0-0--00-',path);
		return {
