	...
	<view class="login-container" :data-theme="pageTheme">
		<u-sticky bgColor="transparent" z-index="99999">
			<view class="searchView">
				<up-navbar title="注销账号" bgColor="transparent" @leftClick="toBack" placeholder :fixed="true"></up-navbar>
			</view>
		</u-sticky>
		<view class="from">
			<view class="phone">
			...
			<view class="password">
				<up-input placeholder="请输入验证码" type='number'  border="surround" v-model="mobileCode" fontSize='16' shape='circle'></up-input>
				<text class="rules-text">账号为空</text>
			</view>
			<view class="password">
				<up-input placeholder="请输入注销原因"  border="surround" v-model="remake" fontSize='16' shape='circle'></up-input>
			</view>
		</view>
		<!-- 登录按钮区域 -->
...
<script setup lang="ts">
	...
	import { cancellationMember,cancellationtCode} from '@/api/login';
	import { getByUserIdAndVehicleAll } from '@/api/user';
	import { forgetPwd } from '@/api/password';
	...
	const getAuthCode = async () => {
		...
		const res = await cancellationtCode(params)
		if (res?.code == '0') {
			startCountdown();
	...
	const doSearch = debounce(() => {
		...
		if(getAlljf.value > 0){
			return uni.showToast({ title: '您还有积分余额，不可注销', icon: 'none' });
		}
		if(vehicleList.value.length > 0){
			return uni.showToast({ title: '你的账号有未解绑车辆，请解绑后再注销', icon: 'none' });
		}
		
		uni.showModal({
			title: '提示',
			content: '注销后数据不可恢复，账户内积分和余额将清零，所有权益失效，请谨慎操作!',
			cancelText: '取消',
			confirmText: '仍要注销',
	...
	const forgetPwdfun= async () => {
		...
		const res = await cancellationMember(params)
		if(res.code==200){
			uni.showToast({
				title: '注销成功',
				icon: 'success',
